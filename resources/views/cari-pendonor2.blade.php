...
@section('content')
        ...
        <form action="{{ route('cari.pendonor2') }}" method="GET" class="row pt-3">
            ...
            <div class="col-lg-2 col-md-3">
                <select id="select-blood-group" placeholder="Golongan Darah" name="bloodType" required>
                    ...
                    <option value="blood-group-a" {{ request()->bloodType == 'blood-group-a' ? 'selected' : '' }}>A
                    </option>
                    <option value="blood-group-b" {{ request()->bloodType == 'blood-group-b' ? 'selected' : '' }}>B
                    </option>
                    <option value="blood-group-o" {{ request()->bloodType == 'blood-group-o' ? 'selected' : '' }}>O
                    </option>
                    <option value="blood-group-ab" {{ request()->bloodType == 'blood-group-ab' ? 'selected' : '' }}>AB
                    </option>
                </select>
            </div>
            ...
            <div class="col-lg-2 col-md-3 pt-2 pt-lg-0 pt-md-0">
                <select id="select-blood-rhesus" placeholder="Rhesus" name="rhesus" required>
                    ...
                    <option value="rhesus-plus" {{ request()->rhesus == 'rhesus-plus' ? 'selected' : '' }}>+</option>
                    <option value="rhesus-minus" {{ request()->rhesus == 'rhesus-minus' ? 'selected' : '' }}>-</option>
                </select>
            </div>
            ...
            <div class="col-lg-2 col-md-3 pt-2 pt-lg-0 pt-md-0">
                <select id="select-city" placeholder="Kota" name="city" required>
                    ...
                    <option value="jakarta utara" {{ request()->city == 'jakarta utara' ? 'selected' : '' }}>Jakarta Utara</option>
                    <option value="jakarta barat" {{ request()->city == 'jakarta barat' ? 'selected' : '' }}>Jakarta Barat</option>
                    <option value="jakarta selatan" {{ request()->city == 'jakarta selatan' ? 'selected' : '' }}>Jakarta Selatan</option>
                    <option value="jakarta timur" {{ request()->city == 'jakarta timur' ? 'selected' : '' }}>Jakarta Timur</option>
                </select>
            </div>
    ...
    <section id="donor-list" class="pt-3 pb-5">
        <div class="container">
            ...
            <h5 class="fw-medium">Daftar pendonor<br>dengan golongan darah <span
                    class="color-1 text-capitalize">{{ explode('-', request()->bloodType)[2] }}</span>
            </h5>
